import officesvg from "../assets/officesome.svg";

export function OfficeSection() {
  return (
    <section className="office">
      <div className="office__content__heading">Our Offices</div>
      <div className="office__content">
        <div className="office__content__svg">
          <img src={officesvg} alt="some svg" />
        </div>
        <div className="office__content__button">
          <button className="office__content__button__one">
            <button className="office__content__button__one__one">
              <svg
                width="42"
                height="60"
                viewBox="0 0 42 60"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M21 0C9.39 0 0 9.39 0 21C0 26.22 1.5 31.11 4.23 35.52C7.08 40.14 10.83 44.1 13.71 48.72C15.12 50.97 16.14 53.07 17.22 55.5C18 57.15 18.63 60 21 60C23.37 60 24 57.15 24.75 55.5C25.86 53.07 26.85 50.97 28.26 48.72C31.14 44.13 34.89 40.17 37.74 35.52C40.5 31.11 42 26.22 42 21C42 9.39 32.61 0 21 0ZM21 29.25C16.86 29.25 13.5 25.89 13.5 21.75C13.5 17.61 16.86 14.25 21 14.25C25.14 14.25 28.5 17.61 28.5 21.75C28.5 25.89 25.14 29.25 21 29.25Z"
                  fill="url(#paint0_linear_3_5)"
                />
                <defs>
                  <linearGradient
                    id="paint0_linear_3_5"
                    x1="21"
                    y1="0"
                    x2="21"
                    y2="60"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop stop-color="#4BD899" />
                    <stop offset="1" stop-color="#5378DE" />
                  </linearGradient>
                </defs>
              </svg>
              <div className="office__content__button__one__content">
                <div className="office__content__button__one__content__heading">
                  Germany
                </div>
                <div className="office__content__button__one__content__info">
                  info@aigron.com <br /> Klosterstrasse 2646446, Emmerich am
                  RheinGermany
                </div>
              </div>
            </button>
          </button>
          <button className="office__content__button__two">
            <button className="office__content__button__one__two">
              <svg
                width="42"
                height="60"
                viewBox="0 0 42 60"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M21 0C9.39 0 0 9.39 0 21C0 26.22 1.5 31.11 4.23 35.52C7.08 40.14 10.83 44.1 13.71 48.72C15.12 50.97 16.14 53.07 17.22 55.5C18 57.15 18.63 60 21 60C23.37 60 24 57.15 24.75 55.5C25.86 53.07 26.85 50.97 28.26 48.72C31.14 44.13 34.89 40.17 37.74 35.52C40.5 31.11 42 26.22 42 21C42 9.39 32.61 0 21 0ZM21 29.25C16.86 29.25 13.5 25.89 13.5 21.75C13.5 17.61 16.86 14.25 21 14.25C25.14 14.25 28.5 17.61 28.5 21.75C28.5 25.89 25.14 29.25 21 29.25Z"
                  fill="url(#paint0_linear_3_5)"
                />
                <defs>
                  <linearGradient
                    id="paint0_linear_3_5"
                    x1="21"
                    y1="0"
                    x2="21"
                    y2="60"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop stop-color="#4BD899" />
                    <stop offset="1" stop-color="#5378DE" />
                  </linearGradient>
                </defs>
              </svg>
              <div className="office__content__button__two__content">
                <div className="office__content__button__two__content__heading">
                  Kuwait
                </div>
                <div className="office__content__button__two__content__info">
                  info@aigron.com <br /> Al-Farwaniya,Am Nimran Complex, Habib
                  Al-Munawar Street, Mezzanine 2, Office 5, Kuwait
                </div>
              </div>
            </button>
          </button>
          <button className="office__content__button__three">
            <button className="office__content__button__one__three">
              <svg
                width="42"
                height="60"
                viewBox="0 0 42 60"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M21 0C9.39 0 0 9.39 0 21C0 26.22 1.5 31.11 4.23 35.52C7.08 40.14 10.83 44.1 13.71 48.72C15.12 50.97 16.14 53.07 17.22 55.5C18 57.15 18.63 60 21 60C23.37 60 24 57.15 24.75 55.5C25.86 53.07 26.85 50.97 28.26 48.72C31.14 44.13 34.89 40.17 37.74 35.52C40.5 31.11 42 26.22 42 21C42 9.39 32.61 0 21 0ZM21 29.25C16.86 29.25 13.5 25.89 13.5 21.75C13.5 17.61 16.86 14.25 21 14.25C25.14 14.25 28.5 17.61 28.5 21.75C28.5 25.89 25.14 29.25 21 29.25Z"
                  fill="url(#paint0_linear_3_5)"
                />
                <defs>
                  <linearGradient
                    id="paint0_linear_3_5"
                    x1="21"
                    y1="0"
                    x2="21"
                    y2="60"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop stop-color="#4BD899" />
                    <stop offset="1" stop-color="#5378DE" />
                  </linearGradient>
                </defs>
              </svg>
              <div className="office__content__button__three__content">
                <div className="office__content__button__three__content__heading">
                  Netherlands
                </div>
                <div className="office__content__button__three__content__info">
                  info@aigron.com <br />
                  Meander 251, 6825 MC Arnhem, the Netherlands
                </div>
              </div>
            </button>
          </button>
          <button className="office__content__button__four">
            <button className="office__content__button__one__four">
              <svg
                width="42"
                height="60"
                viewBox="0 0 42 60"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M21 0C9.39 0 0 9.39 0 21C0 26.22 1.5 31.11 4.23 35.52C7.08 40.14 10.83 44.1 13.71 48.72C15.12 50.97 16.14 53.07 17.22 55.5C18 57.15 18.63 60 21 60C23.37 60 24 57.15 24.75 55.5C25.86 53.07 26.85 50.97 28.26 48.72C31.14 44.13 34.89 40.17 37.74 35.52C40.5 31.11 42 26.22 42 21C42 9.39 32.61 0 21 0ZM21 29.25C16.86 29.25 13.5 25.89 13.5 21.75C13.5 17.61 16.86 14.25 21 14.25C25.14 14.25 28.5 17.61 28.5 21.75C28.5 25.89 25.14 29.25 21 29.25Z"
                  fill="url(#paint0_linear_3_5)"
                />
                <defs>
                  <linearGradient
                    id="paint0_linear_3_5"
                    x1="21"
                    y1="0"
                    x2="21"
                    y2="60"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop stop-color="#4BD899" />
                    <stop offset="1" stop-color="#5378DE" />
                  </linearGradient>
                </defs>
              </svg>
              <div className="office__content__button__four__content">
                <div className="office__content__button__four__content__heading">
                  Pakistan
                </div>
                <div className="office__content__button__four__content__info">
                  info@aigron.com <br />
                  33-Z, 1st Floor Commercial Plaza, DHA Phase III, Lahore,
                  Pakistan.
                </div>
              </div>
            </button>
          </button>
        </div>
      </div>
    </section>
  );
}
